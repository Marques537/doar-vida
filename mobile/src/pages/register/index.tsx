    ...
    if (name != '' && email != '' && password != '') {
      try {
        ...
        if (response.userId < 1 || response.userId === undefined) {
          if (response.message != '') {
            return Alert.alert('Erro', response.message, [{ text: 'OK' }]);
          }
          return Alert.alert(
            'Erro',
            'Ocorreu um erro na tentativa de cadastro. Tente novamente',
            [{ text: 'OK' }]
          );
        }
        Alert.alert('Sucesso', 'Cadastro realizado', [{ text: 'OK' }]);
        handleNavigateBack();
      } catch (err) {
        console.log(err);
        Alert.alert(
